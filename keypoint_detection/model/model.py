...
class KeypointDetectionModel(nn.Module):
    ...
    def forward(self, source_img, tar_img, get_src_kp = False):
        ...
        if get_src_kp:
            src_heatmaps = self.location_encoder(source_img)
            src_keypoints = get_keypoints(heatmaps_to_keypoints(src_heatmaps), self.img_height, self.img_width)
