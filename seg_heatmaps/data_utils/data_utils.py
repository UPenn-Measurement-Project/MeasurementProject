...
#datapoint class
class DataPoint:
    def __init__(self, img_dir, img_file, img_flip, df, img_width, img_height, aug_rot = 0, aug_scale = 1, aug_crop = 1, aug_noise = 0):
        ...
        msk[:, :, int(self.img_height * aug_noise):, int(self.img_width * aug_noise):] = 0
        inv_msk = 1 - msk
        #noise = torch.rand_like(img)
        noise = torch.zeros_like(img)
        img = img * inv_msk + noise * msk
...
class DataProcessor:
    def __init__(self, measurement_file, train_val_split, batch_size, img_width, img_height, seed):
    ...
    def create_ds(self, ds_name, rot_rng = (0, 0, 1), scale_rng = (1, 1, 1), crop_rng = (1, 1, 1), noise_rng = (0, 0, 1)):
        if ds_name == "train":
            ds = self.train_img
                ...
                for aug_rot in np.arange(rot_rng[0], rot_rng[1] + 1e-8, rot_rng[2]):
                    for aug_scale in np.arange(scale_rng[0], scale_rng[1] + 1e-8, scale_rng[2]):
                        for aug_crop in np.arange(crop_rng[0], crop_rng[1] + 1e-8, crop_rng[2]):
                            for aug_noise in np.arange(noise_rng[0], noise_rng[1] + 1e-8, noise_rng[2]):
                                data_points.append(DataPoint(self.img_dir, img_file, img_flip, self.df, self.img_width, self.img_height, aug_rot, aug_scale, aug_crop, aug_noise))
