...
from cnn_backbone.data_utils.data_utils import DataProcessor
from cnn_backbone.model.model import CNNModel, model_to_coord, measurements_to_coord, coord_to_measurements
...
#model set up
model = CNNModel(img_width, img_height, ae_ch_cnt = [1, 64, 128, 32, 1], device = device)
if load_path:
    model.load_state_dict(torch.load(f"./cnn_backbone/model_saves/{load_path}"))
...
for epoch in range(epoch_cnt):
    ...
    if total_loss > best_val_loss:
        ...
        torch.save(model.state_dict(), './cnn_backbone/model_saves/current_best.pth')

    if early_stop_cnt >= early_stop_lim:
        print('EARLY STOPPED')
        break

model.load_state_dict(torch.load("./cnn_backbone/model_saves/current_best.pth"))
torch.save(model.state_dict(), "./cnn_backbone/model_saves/final_model.pth")
