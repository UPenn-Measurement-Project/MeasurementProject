...
class DataPoint:
    def __init__(self, xrayfi, segfi, og_width, og_height, tar_width, tar_height):
        ...
        xrayimg = self.proc_img(xrayimg)

        segimg = nib.load(segfi).get_fdata().transpose(1,0,2).squeeze()
        segimg = self.proc_img(segimg)
...
class DataProcessor:
    def __init__(self, train_val_split, batch_size, og_width, og_height, tar_width, tar_height, seed):
    ...
    def create_ds(self, ds_name):
        ...
        for img_file in tqdm(ds, unit = "image"):
            data_points.append(DataPoint(self.xrayfi + img_file + ".dcm", self.segfi + img_file + ".nii.gz", self.og_width, self.og_height, self.tar_width, self.tar_height))
