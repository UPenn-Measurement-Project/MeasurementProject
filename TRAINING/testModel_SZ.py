...
def test_measurements_to_coord(measurements, ab, pix_per_mm, img_scale_factor, DEBUG_MODE = False):
    #units: mm -> pixels
    ...
    return coord
def test_coord_to_measurements(coord, pix_per_mm, img_scale_factor):
    #units: pixels -> mm
...
class testCNNModel(nn.Module):
    def __init__(self, img_height, img_width):
        ...
        self.seq = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(256 * (img_height // 16) * (img_width // 16), 9)
            )
        '''
        self.conv = nn.Conv2d(1, 16, 3, padding = 1)
        #(# of channels, # of filters, pixel_size, padding = 1)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(2)#reduce image size
        self.flatten = nn.Flatten()
        self.ln = nn.Linear(256 * (img_width // 16) * (img_height // 16), 4096)
