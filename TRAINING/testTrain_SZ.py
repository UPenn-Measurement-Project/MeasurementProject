...
from model.testModel_SZ import testCNNModel

#==========#

#device

#Commented out section: Original GPU Code
'''
...
for epoch in range(epoch_cnt):
    ...
    with torch.no_grad():
        for images, yvals, aug_scales in val_loader:
            ...
            total_percent_err += torch.sum(percent_err, dim=0)
            
        print("model out (first set): ", model_out[0])
        print("real measurements (first set): ", yvals[0])
        print()
    ...
    if total_loss > best_val_loss:
        ...
        torch.save(model.state_dict(), './model_saves/current_best_SZ.pth')
        print('CURRENT BEST UPDATED')
...
model.load_state_dict(torch.load("./model_saves/current_best_SZ.pth", weights_only=True))
torch.save(model.state_dict(), "./model_saves/final_model_SZ.pth")
