...
from model.testModel_SZ import testCNNModel, test_measurements_to_coord
...
print("Using Intel GPU (DirectML):", device)
'''
...
parser.add_argument("--path", type = str, default = "current_best_SZ.pth", required = False, help = "Model path (from ./model_saves/)")
...
#model set up
model = testCNNModel(img_width, img_height)
...
with torch.no_grad():
    for idx, (images, yvals, aug_scales) in enumerate(tqdm(dataloader, unit = "batch")):
        ...
        model_coord = test_measurements_to_coord(model_out,pix_per_mm, img_scale_factor)
        real_coord = test_measurements_to_coord(yvals, pix_per_mm, img_scale_factor)
        ypred = model_out
        #-----------------
        ...
        figT = plt.figure(figsize = (6, 3))#create temporary figure
        ...
        for i, label in enumerate(['D', 'F', 'G', 'W', 'X', 'Y', 'Z', 'A\'', 'A\'\'', 'C']):
