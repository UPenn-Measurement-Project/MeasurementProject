...
parser.add_argument("--path", type = str, default = "current_best.pth", required = False, help = "Model path (from ./dual_heatmaps/model_saves/)")
...
model.load_state_dict(torch.load(f"./dual_heatmaps/model_saves/{model_path}", map_location = "cpu"))
print(f"\n==========\n\nModel loaded from ./dual_heatmaps/model_saves/{model_path}")
...
os.makedirs("./dual_heatmaps/test_results/img", exist_ok=True)
with torch.no_grad():
    for idx, (images, yvals, aug_scales) in enumerate(tqdm(dataloader, unit = "batch")):
        ...
        plt.savefig(f"./dual_heatmaps/test_results/img/{idx}.png")
...
if make_csv:
    ...
    df.to_csv("./dual_heatmaps/test_results/results.csv", index = False)
