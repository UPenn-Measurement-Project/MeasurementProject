...
parser.add_argument("--path", type = str, default = "current_best.pth", required = False, help = "Model path (from ./points_with_seg/model_saves/)")
...
model.load_state_dict(torch.load(f"./points_with_seg/model_saves/{model_path}"))
print(f"\n==========\n\nModel loaded from./points_with_seg/model_saves/{model_path}")
...
with torch.no_grad():
    for idx, (images, yvals, aug_scales) in enumerate(tqdm(dataloader, unit = "batch")):
        ...
        #get seg mask from model
        seg_mask = model.seg_model(images)
        ...
        #2: seg mask
        plt.subplot(1, 4, 2)
        plt.imshow(seg_mask[0][0].detach().cpu(), cmap = 'gray')
        plt.title('Seg Mask')

        #3: image masked by seg
        plt.subplot(1, 4, 3)
        plt.imshow((images[0][0] * seg_mask[0][0]).detach().cpu(), cmap = 'gray')
        plt.title('Seg Channel')

        #4: predicted vs real keypoints
        plt.subplot(1, 4, 4)
        ...
        plt.xlabel('x (pixels)')
        plt.ylabel('y (pixels)')
        plt.title('Keypoints (red=pred, blue=real)')

        plt.tight_layout()
        plt.savefig(f"./points_with_seg/test_results/img/{idx}.png")
...
if make_csv:
    ...
    df.to_csv("./points_with_seg/test_results/results.csv", index = False)
