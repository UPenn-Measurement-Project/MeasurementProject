...
from points_with_seg.data_utils.data_utils import DataProcessor
from points_with_seg.model.model import SimpleCNNModel, model_to_coord, measurements_to_coord, coord_to_measurements
...
parser.add_argument("--exclude_pts", type = str, default = "", required = False, help = "Comma-separated point indices to exclude from loss (e.g. '1' to exclude F)")
...
#model set up
seg_model_path = "./SEGMENTATIONS/mod_seg/model_saves/segunet.pth"
model = SimpleCNNModel(img_width, img_height, batch_norm_setting, seg_model_path)

if load_path:
    model.load_state_dict(torch.load(f"./points_with_seg/model_saves/{load_path}"))

#loss function
exclude_pts = set(int(x) for x in args.exclude_pts.split(",") if x.strip()) if args.exclude_pts else set()
loss_mask = [i for i in range(12) if i not in exclude_pts]
print(f"Loss point indices: {loss_mask} (excluded: {sorted(exclude_pts) if exclude_pts else 'none'})\n")
lossfn = lambda ypred, yvals, aug_scales : torch.mean(torch.sum(torch.norm(ypred[:, loss_mask] - yvals[:, loss_mask], dim = 2), dim = 1) / aug_scales)
...
for epoch in range(epoch_cnt):
    ...
    if total_loss > best_val_loss:
        ...
        torch.save(model.state_dict(), './points_with_seg/model_saves/current_best.pth')

    if early_stop_cnt >= early_stop_lim:
        print('EARLY STOPPED')
        break

model.load_state_dict(torch.load("./points_with_seg/model_saves/current_best.pth"))
torch.save(model.state_dict(), "./points_with_seg/model_saves/final_model.pth")
